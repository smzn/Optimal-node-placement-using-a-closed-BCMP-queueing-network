...
class BCMP_GA_Class:
    def __init__(self, N, R, K_total, node_number, npop, ngen, crosspb, mutpb, rank, size, comm, popularity_file, distance_file):
        self.N = N
        self.R = R
        ...
        if self.rank == 0:
            ...
            self.popularity = np.array(pd.read_csv(popularity_file, header=None)) #2022/11/18 ファイル取り込みに変更
            #self.popularity = self.getPopurarity(self.N, self.R, prate) #人気度を設定
            self.distance_matrix = np.array(pd.read_csv(distance_file, header=None)) #2022/11/18 ファイル取り込みに変更
            ...
            #np.savetxt('./popularity_std_'+str(self.N)+'_'+str(self.R)+'_'+str(self.K_total)+'_'+str(self.node_number)+'_'+str(self.npop)+'_'+str(self.ngen)+'.csv', np.array(self.popularity), delimiter=',', fmt='%d')
            #np.savetxt('./distance_std_'+str(self.N)+'_'+str(self.R)+'_'+str(self.K_total)+'_'+str(self.node_number)+'_'+str(self.npop)+'_'+str(self.ngen)+'.csv', self.distance_matrix, delimiter=',', fmt='%.5f')
        else:
            self.pool = [[]]
    ...
    def getFinalResult(self, individual):
        ...
        np.savetxt('./ga_L_std_'+str(self.N)+'_'+str(self.R)+'_'+str(self.K_total)+'_'+str(self.node_number)+'_'+str(self.npop)+'_'+str(self.ngen)+'.csv', L_class, delimiter=',')
        np.savetxt('./ga_Node_std_'+str(self.N)+'_'+str(self.R)+'_'+str(self.K_total)+'_'+str(self.node_number)+'_'+str(self.npop)+'_'+str(self.ngen)+'.csv', individual, delimiter=',')
        np.savetxt('./ga_P_std_'+str(self.N)+'_'+str(self.R)+'_'+str(self.K_total)+'_'+str(self.node_number)+'_'+str(self.npop)+'_'+str(self.ngen)+'.csv', p, delimiter=',')
        np.savetxt('./ga_Object_std_'+str(self.N)+'_'+str(self.R)+'_'+str(self.K_total)+'_'+str(self.node_number)+'_'+str(self.npop)+'_'+str(self.ngen)+'.csv', np.array(self.bestfit_seriese), delimiter=',')
        print('Final Result')
...
#        return distance_matrix
   ...
   #重力モデルで推移確率行列を作成 
    ...
    def getGraph(self):
        ...
        plt.grid()
        plt.legend()
        fig.savefig('./ga_transition_std_'+str(self.N)+'_'+str(self.R)+'_'+str(self.K_total)+'_'+str(self.node_number)+'_'+str(self.npop)+'_'+str(self.ngen)+'.png')
    ...
    def getDistance(self, N, dim):
        ...
        np.savetxt('./position_std_'+str(self.N)+'_'+str(self.R)+'_'+str(self.K_total)+'_'+str(self.node_number)+'_'+str(self.npop)+'_'+str(self.ngen)+'.csv', position, delimiter=',', fmt='%d')
...
if __name__ == '__main__':
    ...
    popularity_file = sys.argv[7]
    distance_file = sys.argv[8]
    crosspb = 0.5
    mutpb = 0.2
    start = time.time()
    bcmp = BCMP_GA_Class(N, R, K_total, node_number, npop, ngen, crosspb, mutpb, rank, size, comm, popularity_file, distance_file)
